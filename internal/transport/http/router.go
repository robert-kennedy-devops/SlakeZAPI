...
func NewRouter(
	...
	billingH := handler.NewBillingHandler(billingUC, log)
	...
	groupH := handler.NewGroupHandler(waUC, msgUC)
	...
	mux.HandleFunc("POST /billing/stripe/webhook", billingH.StripeWebhook)
	...
	mux.Handle("GET /app/whatsapp/profile", withUserRole(waH.GetProfile, domain.UserRoleOwner, domain.UserRoleAdmin, domain.UserRoleOperator, domain.UserRoleViewer))
	mux.Handle("PATCH /app/whatsapp/profile", withUserRole(waH.UpdateProfile, domain.UserRoleOwner, domain.UserRoleAdmin))
	mux.Handle("GET /app/whatsapp/privacy", withUserRole(waH.GetPrivacy, domain.UserRoleOwner, domain.UserRoleAdmin))
	mux.Handle("PATCH /app/whatsapp/privacy", withUserRole(waH.UpdatePrivacy, domain.UserRoleOwner, domain.UserRoleAdmin))
	mux.Handle("POST /app/whatsapp/pair-phone", withUserRole(waH.PairPhone, domain.UserRoleOwner, domain.UserRoleAdmin))
	mux.Handle("POST /app/whatsapp/restart", withUserRole(waH.Restart, domain.UserRoleOwner, domain.UserRoleAdmin))
	mux.Handle("POST /app/contacts/{phone}/block", withUserRole(waH.BlockContact, domain.UserRoleOwner, domain.UserRoleAdmin, domain.UserRoleOperator))
	mux.Handle("DELETE /app/contacts/{phone}/block", withUserRole(waH.UnblockContact, domain.UserRoleOwner, domain.UserRoleAdmin, domain.UserRoleOperator))
	mux.Handle("GET /app/contacts/{phone}/avatar", withUserRole(waH.GetContactAvatar, domain.UserRoleOwner, domain.UserRoleAdmin, domain.UserRoleOperator, domain.UserRoleViewer))
	mux.Handle("POST /app/chats/{phone}/archive", withUserRole(waH.ArchiveChat, domain.UserRoleOwner, domain.UserRoleAdmin, domain.UserRoleOperator))
	mux.Handle("POST /app/chats/{phone}/mute", withUserRole(waH.MuteChat, domain.UserRoleOwner, domain.UserRoleAdmin, domain.UserRoleOperator))
	mux.Handle("POST /app/chats/{phone}/pin", withUserRole(waH.PinChat, domain.UserRoleOwner, domain.UserRoleAdmin, domain.UserRoleOperator))
	mux.Handle("POST /app/chats/{phone}/read", withUserRole(waH.MarkChatRead, domain.UserRoleOwner, domain.UserRoleAdmin, domain.UserRoleOperator))
	...
	mux.Handle("POST /app/messages/send-location", withUserRole(msgH.SendLocation, domain.UserRoleOwner, domain.UserRoleAdmin, domain.UserRoleOperator))
	mux.Handle("POST /app/messages/send-contact", withUserRole(msgH.SendContactCard, domain.UserRoleOwner, domain.UserRoleAdmin, domain.UserRoleOperator))
	mux.Handle("POST /app/messages/send-sticker", withUserRole(msgH.SendSticker, domain.UserRoleOwner, domain.UserRoleAdmin, domain.UserRoleOperator))
	mux.Handle("POST /app/messages/send-quoted", withUserRole(msgH.SendQuoted, domain.UserRoleOwner, domain.UserRoleAdmin, domain.UserRoleOperator))
	mux.Handle("POST /app/messages/react", withUserRole(msgH.React, domain.UserRoleOwner, domain.UserRoleAdmin, domain.UserRoleOperator))
	mux.Handle("POST /app/messages/delete", withUserRole(msgH.Delete, domain.UserRoleOwner, domain.UserRoleAdmin, domain.UserRoleOperator))
	mux.Handle("POST /app/messages/edit", withUserRole(msgH.Edit, domain.UserRoleOwner, domain.UserRoleAdmin, domain.UserRoleOperator))
	mux.Handle("POST /app/messages/forward", withUserRole(msgH.Forward, domain.UserRoleOwner, domain.UserRoleAdmin, domain.UserRoleOperator))
	mux.Handle("POST /app/messages/star", withUserRole(msgH.Star, domain.UserRoleOwner, domain.UserRoleAdmin, domain.UserRoleOperator))
	...
	mux.Handle("GET /app/contacts", withUserRole(msgH.ListContacts, domain.UserRoleOwner, domain.UserRoleAdmin, domain.UserRoleOperator, domain.UserRoleViewer))
	...
	mux.Handle("GET /app/groups", withUserRole(groupH.List, domain.UserRoleOwner, domain.UserRoleAdmin, domain.UserRoleOperator, domain.UserRoleViewer))
	mux.Handle("POST /app/groups", withUserRole(groupH.Create, domain.UserRoleOwner, domain.UserRoleAdmin, domain.UserRoleOperator))
	mux.Handle("GET /app/groups/{jid}", withUserRole(groupH.Get, domain.UserRoleOwner, domain.UserRoleAdmin, domain.UserRoleOperator, domain.UserRoleViewer))
	mux.Handle("PATCH /app/groups/{jid}", withUserRole(groupH.UpdateInfo, domain.UserRoleOwner, domain.UserRoleAdmin, domain.UserRoleOperator))
	mux.Handle("POST /app/groups/{jid}/participants", withUserRole(groupH.UpdateParticipants, domain.UserRoleOwner, domain.UserRoleAdmin, domain.UserRoleOperator))
	mux.Handle("GET /app/groups/{jid}/invite", withUserRole(groupH.InviteLink, domain.UserRoleOwner, domain.UserRoleAdmin, domain.UserRoleOperator))
	mux.Handle("POST /app/groups/{jid}/leave", withUserRole(groupH.Leave, domain.UserRoleOwner, domain.UserRoleAdmin, domain.UserRoleOperator))
	...
	mux.Handle("GET /app/messages/{id}/transcript", withUserRole(msgH.GetTranscript, domain.UserRoleOwner, domain.UserRoleAdmin, domain.UserRoleOperator, domain.UserRoleViewer))
	...
	mux.Handle("GET /app/billing/subscription", withUserRole(billingH.GetSubscription, domain.UserRoleOwner, domain.UserRoleAdmin, domain.UserRoleOperator, domain.UserRoleViewer))
	mux.Handle("POST /app/billing/checkout", withUserRole(billingH.CreateCheckout, domain.UserRoleOwner, domain.UserRoleAdmin))
	mux.Handle("POST /app/billing/portal", withUserRole(billingH.OpenPortal, domain.UserRoleOwner, domain.UserRoleAdmin))
	mux.Handle("POST /app/billing/change-plan", withUserRole(billingH.ChangePlan, domain.UserRoleOwner, domain.UserRoleAdmin))
	mux.Handle("POST /app/billing/cancel", withUserRole(billingH.CancelSubscription, domain.UserRoleOwner, domain.UserRoleAdmin))
	...
	mux.Handle("GET /whatsapp/profile", withAuth(waH.GetProfile))
	mux.Handle("PATCH /whatsapp/profile", withAuth(waH.UpdateProfile))
	mux.Handle("GET /whatsapp/privacy", withAuth(waH.GetPrivacy))
	mux.Handle("PATCH /whatsapp/privacy", withAuth(waH.UpdatePrivacy))
	mux.Handle("POST /whatsapp/pair-phone", withAuth(waH.PairPhone))
	mux.Handle("POST /whatsapp/restart", withAuth(waH.Restart))
	...
	mux.Handle("POST /messages/send-location", withAuth(msgH.SendLocation))
	mux.Handle("POST /messages/send-contact", withAuth(msgH.SendContactCard))
	mux.Handle("POST /messages/send-sticker", withAuth(msgH.SendSticker))
	mux.Handle("POST /messages/send-quoted", withAuth(msgH.SendQuoted))
	mux.Handle("POST /messages/react", withAuth(msgH.React))
	mux.Handle("POST /messages/delete", withAuth(msgH.Delete))
	mux.Handle("POST /messages/edit", withAuth(msgH.Edit))
	mux.Handle("POST /messages/forward", withAuth(msgH.Forward))
	mux.Handle("POST /messages/star", withAuth(msgH.Star))
	mux.Handle("POST /status/post", withAuth(msgH.StatusPost))
	mux.Handle("POST /contacts/resolve", withAuth(msgH.ResolveContacts))
	mux.Handle("GET /contacts", withAuth(msgH.ListContacts))
	mux.Handle("POST /contacts/{phone}/block", withAuth(waH.BlockContact))
	mux.Handle("DELETE /contacts/{phone}/block", withAuth(waH.UnblockContact))
	mux.Handle("GET /contacts/{phone}/avatar", withAuth(waH.GetContactAvatar))
	mux.Handle("POST /chats/{phone}/archive", withAuth(waH.ArchiveChat))
	mux.Handle("POST /chats/{phone}/mute", withAuth(waH.MuteChat))
	mux.Handle("POST /chats/{phone}/pin", withAuth(waH.PinChat))
	mux.Handle("POST /chats/{phone}/read", withAuth(waH.MarkChatRead))
	...
	mux.Handle("GET /groups", withAuth(groupH.List))
	mux.Handle("POST /groups", withAuth(groupH.Create))
	mux.Handle("GET /groups/{jid}", withAuth(groupH.Get))
	mux.Handle("PATCH /groups/{jid}", withAuth(groupH.UpdateInfo))
	mux.Handle("POST /groups/{jid}/participants", withAuth(groupH.UpdateParticipants))
	mux.Handle("GET /groups/{jid}/invite", withAuth(groupH.InviteLink))
	mux.Handle("POST /groups/{jid}/leave", withAuth(groupH.Leave))
	mux.Handle("GET /messages/{id}", withAuth(msgH.Get))
	mux.Handle("GET /messages/{id}/media", withAuth(msgH.GetMedia))
	mux.Handle("GET /messages/{id}/transcript", withAuth(msgH.GetTranscript))
	...
	mux.Handle("GET /billing/subscription", withAuth(billingH.GetSubscription))
	mux.Handle("POST /billing/checkout", withAuth(billingH.CreateCheckout))
	mux.Handle("POST /billing/portal", withAuth(billingH.OpenPortal))
	mux.Handle("POST /billing/change-plan", withAuth(billingH.ChangePlan))
	mux.Handle("POST /billing/cancel", withAuth(billingH.CancelSubscription))
